...
std::function<void(NotificationActivatedEventArgsCompat)> _onActivated = nullptr;


void NotificationManager::Register(std::wstring aumid, std::wstring displayName, std::wstring iconPath)
{
	// If has identity
	if (HasIdentity())
	{
...
void NotificationManager::OnActivated(std::function<void(NotificationActivatedEventArgsCompat)> callback)
{
	EnsureRegistered();
...
ToastNotifier NotificationManager::CreateToastNotifier()
{
	if (HasIdentity())
	{
...
void NotificationManager::Uninstall()
{
	if (IsContainerized())
	{
...
struct callback : implements<callback, INotificationActivationCallback>
{
		...
		[[maybe_unused]] ULONG dataCount) noexcept
	{
		if (_onActivated != nullptr)
		{
			...
			NotificationActivatedEventArgsCompat args(argument, userInput);
			_onActivated(args);
		}
		return S_OK;
	}
...
NotificationHistoryCompat NotificationManager::History()
{
	EnsureRegistered();

	NotificationHistoryCompat history(_win32Aumid);
	return history;
}

void NotificationHistoryCompat::Clear()
{
	if (_win32Aumid.empty())
	{
		_history.Clear();
...
IVectorView<ToastNotification> NotificationHistoryCompat::GetHistory()
{
	if (_win32Aumid.empty())
	{
...
void NotificationHistoryCompat::Remove(std::wstring tag)
{
	if (_win32Aumid.empty())
	{
		_history.Remove(tag);
...
void NotificationHistoryCompat::Remove(std::wstring tag, std::wstring group)
{
	if (_win32Aumid.empty())
	{
...
void NotificationHistoryCompat::RemoveGroup(std::wstring group)
{
	if (_win32Aumid.empty())
	{
